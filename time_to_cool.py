...
while True:
  pv = e.read_temp(ser)
  if (UPPER_TEMP - STABILITY_TOL < pv) and (UPPER_TEMP + STABILITY_TOL > pv):
    stable_start = t.perf_counter()
    while (UPPER_TEMP - STABILITY_TOL < pv) and \
      (UPPER_TEMP + STABILITY_TOL > pv):
...
print('Time to ramp temperature from %3.1f to %3.1f is %.2f seconds' \
  % (UPPER_TEMP,LOWER_TEMP,ramp_finish-ramp_start))
